<!--
  header.vue - Barre de navigation responsive
  Fonctionnalités : Menu mobile/desktop, avatar utilisateur, thème, connexion
-->
<template>
  <div class="navbar top-0 fixed bg-base-100 px-4 shadow-lg z-10">
    ...
    <div class="navbar-start">
      <div class="dropdown">
        ...
        <ul class="w-52 menu menu-md dropdown-content mt-3 p-2 bg-base-100 rounded-box shadow">
          <li><NuxtLink to="/#hero" class="text-lg">Home</NuxtLink></li>
          <li><NuxtLink to="/#about" class="text-lg">About</NuxtLink></li>
          <li><NuxtLink to="/#skills" class="text-lg">My Stack</NuxtLink></li>
          <li><NuxtLink to="/#projects" class="text-lg">Projects</NuxtLink></li>
          <li><NuxtLink to="/#contact" class="text-lg">Contact</NuxtLink></li>
        </ul>
      </div>
    ...
    <!-- Section droite : Avatar (si connecté) ou Thème + Login -->
    <div class="navbar-end">
      ...
      <div v-else>
        <label class="swap-rotate swap btn btn-ghost">
          <input type="checkbox" :checked="themeStore.activeTheme === 'winter'" data-toggle-theme="forest,winter" @change="themeStore.changeTheme()" />
...
// URL de l'avatar (priorité : AniList medium > large > local > default)
const avatarUrl = computed(() => {
  return pocketbaseStore.authRecord?.anilist_avatar_url_medium || pocketbaseStore.authRecord?.anilist_avatar_url_large || userData?.avatar || '/img/user.png';
});
